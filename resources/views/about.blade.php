    ...
    <section class="flex overflow-hidden relative justify-center items-center pt-40 pb-24 bg-brand-primary">
        ...
        <div class="container relative z-10 px-4 mx-auto text-center">
            ...
            <p class="mx-auto max-w-2xl text-lg font-medium leading-relaxed opacity-0 transform translate-y-4 text-white/80 animate-fade-in-up"
                ...
                {{ $companySettings->about_subtitle[$locale] ?? __('شريكك الموثوق في البناء والتشييد والخدمات الهندسية منذ أكثر من عقدين.') }}</p>
        </div>
    </section>
    ...
    <section class="py-24 bg-white">
        <div class="container px-4 mx-auto max-w-7xl md:px-6">
            <div class="grid grid-cols-1 gap-16 items-center lg:grid-cols-2">
                ...
                <div class="relative">
                    <div class="overflow-hidden rounded-2xl border border-gray-100 aspect-[4/3] shadow-2xl bg-gray-50 p-2">
                        <img src="{{ $companySettings->about_image ? Storage::url($companySettings->about_image) : 'https://images.unsplash.com/photo-1541888086425-d81bb19240f5?q=80&w=1000&auto=format&fit=crop' }}" alt="Company Profile" class="{{ $companySettings->about_image ? 'object-contain' : 'object-cover' }} w-full h-full rounded-xl">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="container px-4 mx-auto max-w-7xl md:px-6">
            ...
            <div class="grid grid-cols-1 gap-8 md:grid-cols-3">
                @forelse(\App\Models\Certification::where('active', true)->orderBy('sort_order')->get() as $cert)
                    <div class="p-8 text-center bg-white rounded-2xl border border-gray-100 shadow-lg transition-all duration-300 hover:-translate-y-2">
                        @if($cert->image)
                            <img src="{{ $cert->imageUrl('image') }}" alt="{{ $cert->name }}" class="object-contain mx-auto mb-6 h-24">
                        @else
                            <div class="flex justify-center items-center mx-auto mb-6 w-24 h-24 bg-gray-50 rounded-full text-brand-secondary">
                                <x-heroicon-o-shield-check class="w-12 h-12"/>
                            </div>
                        @endif
                        <h3 class="mb-3 text-xl font-bold text-brand-primary">{{ $cert->name }}</h3>
                        <p class="text-sm text-gray-500">{{ $cert->issue_date ? $cert->issue_date->format('Y') : '' }}</p>
                    </div>
                @empty
            ...
            <div class="grid grid-cols-1 gap-16 items-center lg:grid-cols-2">
                <div class="relative order-2 lg:order-1">
                    <div class="overflow-hidden rounded-2xl aspect-[4/3] shadow-2xl border border-brand-primary/50">
                        <img src="{{ $companySettings->hse_image ? Storage::url($companySettings->hse_image) : 'https://images.unsplash.com/photo-1504307651254-35680f356f58?q=80&w=1000&auto=format&fit=crop' }}" alt="HSE" class="object-contain w-full h-full opacity-80 mix-blend-luminosity">
                        <div class="absolute inset-0 mix-blend-multiply bg-brand-primary/40"></div>
                    </div>
                </div>
                
                <div class="flex flex-col order-1 gap-6 lg:order-2">
                    <span class="inline-flex gap-2 items-center px-4 py-2 w-max text-sm font-bold tracking-wide rounded-full text-brand-secondary bg-brand-secondary/20">
