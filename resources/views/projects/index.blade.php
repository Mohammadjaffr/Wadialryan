<x-layouts.app>
    ...
    <section class="flex overflow-hidden relative justify-center items-center pt-40 pb-24 bg-gray-50">
        ...
        <div class="container relative z-10 px-4 mx-auto text-center">
            ...
            <p class="mx-auto max-w-2xl text-lg font-medium leading-relaxed opacity-0 transform translate-y-4 text-gray-600 animate-fade-in-up" style="animation-delay: 200ms;">{{ __('تصفح مجموعة مختارة من أحدث مشاريعنا التي تم إنجازها بكل دقة واحترافية لتجسد رؤية عملائنا.') }}</p>
        </div>  
    </section>
    ...
    <section class="overflow-hidden relative py-24 min-h-screen bg-white border-t border-gray-100" x-data="{ category: 'all', init() { setTimeout(() => window.dispatchEvent(new Event('resize')), 500) } }">
        ...
        <div class="container relative z-10 px-4 mx-auto md:px-6">
            ...
            <div class="flex flex-wrap gap-3 justify-center mb-16 md:gap-4">
                ...
                <button @click="category = '{{ $cat }}'" :class="category === '{{ $cat }}' ? 'bg-brand-secondary text-white shadow-lg' : 'bg-white text-brand-primary border-gray-200 hover:bg-gray-50'" class="px-8 py-3 font-bold rounded-full border transition-all duration-300">{{ $cat }}</button>
                @endforeach
            </div>
            @endif
            ...
            <div class="grid grid-cols-1 gap-8 pb-10 md:grid-cols-2 lg:grid-cols-3">
                @forelse($projects as $project)
                    <div x-show="category === 'all' || category === '{{ $project->category }}'" 
                         ...
                         class="project-card flex flex-col h-full rounded-2xl bg-white border border-gray-100 shadow-lg hover:shadow-xl transition-all duration-500 group overflow-hidden hover:-translate-y-2 cursor-pointer">
                        <a href="/projects/{{ $project->id }}" class="flex flex-col w-full h-full">
                            <div class="relative overflow-hidden aspect-[4/3] w-full bg-gray-100">
                                @if($project->main_image)
                                    <img src="{{ $project->imageUrl('main_image') }}" alt="{{ $project->title }}" class="object-cover relative z-10 w-full h-full transition-transform duration-700 group-hover:scale-105">
                                @else
                                    <img src="https://images.unsplash.com/photo-1541888086425-d81bb19240f5?q=80&w=800&auto=format&fit=crop" alt="{{ $project->title }}" class="object-cover relative z-10 w-full h-full transition-transform duration-700 group-hover:scale-105 opacity-80">
                                    <div class="absolute inset-0 bg-brand-primary/10 mix-blend-overlay"></div>
                                @endif
                            ...
                            <div class="flex relative z-10 flex-col flex-grow p-8 bg-white">
                                ...
                                <p class="flex-grow mb-6 font-medium leading-relaxed text-gray-600 line-clamp-3">{{ strip_tags($project->short_description ?? $project->description) }}</p>
                ...
                @empty
                    <div class="col-span-full text-center py-20 bg-white rounded-2xl border border-gray-100 mt-8 shadow-sm">
                        <h3 class="mb-3 text-2xl font-bold text-brand-primary">{{ __('جاري إضافة المشاريع') }}</h3>
                        <p class="text-lg font-medium text-gray-500">{{ __('لم نتمكن من العثور على مشاريع في هذا التصنيف حالياً.') }}</p>
                    </div>
                @endforelse
            </div>
            
            <!-- Empty State / Fallback (if filtered by Alpine) -->
            <div x-show="!$el.previousElementSibling.querySelector('.project-card[style*=\'display: block\'], .project-card:not([style*=\'display: none\'])')" style="display: none;" class="text-center py-20 bg-white rounded-2xl border border-gray-100 mt-8 shadow-sm">
                ...
                <p class="text-lg font-medium text-gray-500">{{ __('لم نتمكن من العثور على مشاريع في هذا التصنيف.') }}</p>
            </div>
