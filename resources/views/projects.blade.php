<x-layouts.app>
    <x-slot:title>مشاريعنا - وادي الريان للمقاولات</x-slot>
    ...
    <section class="relative pt-40 pb-24 bg-brand-surface flex items-center justify-center overflow-hidden border-b border-white/5">
        <div class="container mx-auto px-4 text-center relative z-10">
            <div class="mb-4">
                <span class="inline-block text-brand-orange font-bold tracking-[0.2em] uppercase text-sm mb-2 px-4 py-1 bg-brand-orange/10 rounded-full">سجل الإنجازات</span>
            </div>
            <h1 class="text-4xl md:text-6xl font-extrabold text-white mb-6">معرض المشاريع</h1>
            <p class="text-lg text-gray-400 max-w-2xl mx-auto font-medium leading-relaxed">تصفح مجموعة مختارة من أحدث مشاريعنا التي تم إنجازها بكل دقة واحترافية لتجسد رؤية عملائنا.</p>
        </div>
    </section>
    ...
    <section class="py-24 bg-brand-dark min-h-screen relative z-10" x-data="{ category: 'all', init() { setTimeout(() => ScrollTrigger.refresh(), 500) } }">
        <div class="container mx-auto px-4 md:px-6">
            ...
            <div class="flex justify-center gap-3 md:gap-4 mb-16 flex-wrap">
                <button @click="category = 'all'" :class="category === 'all' ? 'bg-brand-orange text-brand-dark border-brand-orange' : 'bg-brand-surface text-gray-400 border-gray-800 hover:border-brand-orange hover:text-white'" class="px-8 py-2.5 rounded-full font-bold transition-all duration-300 border shadow-sm">الكل</button>
                <button @click="category = 'تجاري'" :class="category === 'تجاري' ? 'bg-brand-orange text-brand-dark border-brand-orange' : 'bg-brand-surface text-gray-400 border-gray-800 hover:border-brand-orange hover:text-white'" class="px-8 py-2.5 rounded-full font-bold transition-all duration-300 border shadow-sm">تجاري</button>
                <button @click="category = 'سكني'" :class="category === 'سكني' ? 'bg-brand-orange text-brand-dark border-brand-orange' : 'bg-brand-surface text-gray-400 border-gray-800 hover:border-brand-orange hover:text-white'" class="px-8 py-2.5 rounded-full font-bold transition-all duration-300 border shadow-sm">سكني</button>
                <button @click="category = 'بنية تحتية'" :class="category === 'بنية تحتية' ? 'bg-brand-orange text-brand-dark border-brand-orange' : 'bg-brand-surface text-gray-400 border-gray-800 hover:border-brand-orange hover:text-white'" class="px-8 py-2.5 rounded-full font-bold transition-all duration-300 border shadow-sm">بنية تحتية</button>
            </div>
            ...
            <div x-show="!$el.previousElementSibling.querySelector('.project-card[style*=\'display: block\'], .project-card:not([style*=\'display: none\'])')" style="display: none;" class="text-center py-20 bg-brand-surface rounded-2xl border border-gray-800 mt-8">
                ...
                <p class="text-gray-400 font-medium">لم نتمكن من العثور على مشاريع في هذا التصنيف.</p>
            </div>
